...
SELECT P.Desc_Producto , SUM(V.Cantidad_Vendida) AS [Sum Cant vendida] 
...
ORDER BY [Sum Cant vendida] DESC;
...
SELECT C.Desc_Categoria, SUM(V.Monto_Vendido) AS [Sum Monto vendido] 
FROM DB_COMERCIAL.dbo.Ventas AS V
	JOIN DB_COMERCIAL.dbo.Categoria AS C
		ON C.COD_Categoria = V.COD_CATEGORIA
GROUP BY C.Desc_Categoria
ORDER BY [Sum Monto vendido] DESC;
...
SELECT TOP (3) 
	...
	SUM(FV.Monto_Vendido) AS [Total vendido]
...
SELECT TOP (3) 
	...
	SUM(V.Monto_Vendido) AS [Total comprado]
FROM DB_COMERCIAL.dbo.Ventas AS V
...
SELECT TOP (3) 
	...
	SUM(V.Monto_Vendido) AS [Ingresos totales]
FROM DB_COMERCIAL.dbo.Ventas AS V
	JOIN DB_COMERCIAL.dbo.Producto AS P
		ON P.COD_Producto = V.COD_PRODUCTO
